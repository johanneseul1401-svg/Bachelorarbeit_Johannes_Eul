...
reg_anderer_zeitraum <- feols(
  fml = log_Beschäftigung ~ Exposure*Post 
  | Code + Jahr:Quartal, 
  data = df_data_processed_3,
  cluster = ~Code)

summary(reg_anderer_zeitraum)

modelsummary(
  
  reg_anderer_zeitraum,
  ...
  title = "Difference-in-Differences Regression mit alternativem Zeitraum (2017-2024)",
  
  output = "outputs/reg_anderer_zeitraum.tex",
